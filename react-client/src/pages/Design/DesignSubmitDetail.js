                ...
                {design ? (
                    <>
                        ...
                        >
                            ...
                            {design.filePaths && design.filePaths.length > 0 && (
                                <>
                                    {/* Viewer로 첫 번째 파일 표시 */}
                                    {isPdfFile(design.filePaths[0]) && !hasError ? (
                                        <Worker workerUrl="https://unpkg.com/pdfjs-dist@2.16.105/build/pdf.worker.min.js">
                                            <ViewerWrapper className="viewer-container" tabIndex={0}>
                                                <Viewer
                                                    fileUrl={`http://localhost:8080/${design.filePaths[0]}`}
                                                    plugins={[
                                    ...
                                    {design.filePaths[1] && (
                                        ...
                                        <p>
                                            <strong>첨부 자료:</strong>{" "}
                                            <a
                                                href={`http://localhost:8080/${design.filePaths[1]}`}
                                                download={design.fileNames[1]}
                                                style={{
                                                    color: "blue",                                                    
                                                }}
                                            >
                        ...
                        <InfoBox>
                            <p>
                                <strong>소감문:</strong> {design.thought}
                            </p>
                            <p>
                                <strong>작성일:</strong>{" "}
                            ...
                            <p>
                                <strong>수정일:</strong>{" "}
                                {new Date(design.updatedAt).toLocaleDateString("ko-KR")}
                            </p>
                            <p>
                                <strong>팀명:</strong> {design.teamName}
                            </p>
                            <p>
                                <strong>팀원:</strong> {design.member}
                            </p>
                        </InfoBox>
                        <OptionsIcon
                            onClick={(e) => {
                        ...
                        <OptionsMenu ref={optionsRef} visible={visibleOptions}>
                            <button onClick={() => navigate(`/design/submit/register/${id}`)}>수정</button>
