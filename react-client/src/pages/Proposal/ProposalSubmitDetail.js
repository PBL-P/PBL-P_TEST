                    ...
                    <>
                        ...
                        >
                            <h4 style={{ marginBottom: "18px" }}>
                                ...
                                {proposal.fileNames[0]}
                            </h4>
                            {proposal.filePaths && proposal.filePaths.length > 0 && (
                                <>
                                    {/* Viewer로 첫 번째 파일 표시 */}
                                    {isPdfFile(proposal.filePaths[0]) && !hasError ? (
                                        <Worker workerUrl="https://unpkg.com/pdfjs-dist@2.16.105/build/pdf.worker.min.js">
                                            <ViewerWrapper className="viewer-container" tabIndex={0}>
                                                <Viewer
                                                    fileUrl={`http://localhost:8080/${proposal.filePaths[0]}`}
                                                    plugins={[
                                    ...
                                    {/* 두 번째 파일 다운로드 링크로 표시 */}
                                    {proposal.filePaths[1] && (
                                        <div style={{ marginTop: "16px", display:"flex"}}>
                                        <p>
                                            <strong>첨부 자료:</strong>{" "}
                                            <a
                                                href={`http://localhost:8080/${proposal.filePaths[1]}`}
                                                download={proposal.fileNames[1]}
                                                style={{
                                                    color: "blue",                                                    
                                                }}
                                            >
                                                {proposal.fileNames[1]}
                                            </a>
                                        </p>
                                        </div>
                                    )}
                        ...
                        <InfoBox>
                            <p>
                                <strong>소감문:</strong> {proposal.thought}
                            </p>
                            <p>
                                <strong>작성일:</strong>{" "}
                            ...
                            <p>
                                <strong>팀명:</strong> {proposal.teamName}
                            </p>
                            <p>
                                <strong>팀원:</strong> {proposal.member}
                            </p>
                        </InfoBox>
                        <OptionsIcon
                            onClick={(e) => {
                        ...
                        <OptionsMenu ref={optionsRef} visible={visibleOptions}>
                            <button onClick={() => navigate(`/proposal/submit/register/${id}`)}>수정</button>
