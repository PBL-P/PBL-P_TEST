                ...
                {plan ? (
                    <>
                        ...
                        >
                            ...
                            {plan.filePaths && plan.filePaths.length > 0 && (
                                <>
                                    {/* Viewer로 첫 번째 파일 표시 */}
                                    {isPdfFile(plan.filePaths[0]) && !hasError ? (
                                        <Worker workerUrl="https://unpkg.com/pdfjs-dist@2.16.105/build/pdf.worker.min.js">
                                            <ViewerWrapper className="viewer-container" tabIndex={0}>
                                                <Viewer
                                                    fileUrl={`http://localhost:8080/${plan.filePaths[0]}`}
                                                    plugins={[
                                    ...
                                    {plan.filePaths[1] && (
                                        ...
                                        <p>
                                            <strong>첨부 자료:</strong>{" "}
                                            <a
                                                href={`http://localhost:8080/${plan.filePaths[1]}`}
                                                download={plan.fileNames[1]}
                                                style={{
                                                    color: "blue",                                                    
                                                }}
                                            >
                        ...
                        <InfoBox>
                            <p>
                                <strong>소감문:</strong> {plan.thought}
                            </p>
                            <p>
                                <strong>작성일:</strong>{" "}
                            ...
                            <p>
                                <strong>팀명:</strong> {plan.teamName}
                            </p>
                            <p>
                                <strong>팀원:</strong> {plan.member}
                            </p>
                        </InfoBox>
                        <OptionsIcon
                            onClick={(e) => {
                        ...
                        <OptionsMenu ref={optionsRef} visible={visibleOptions}>
                            <button onClick={() => navigate(`/plan/submit/register/${id}`)}>수정</button>
